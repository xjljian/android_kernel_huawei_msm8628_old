...
unsigned long glove_holster_status = 0;
atomic_t holster_status_flag;
...
int cyttsp4_set_disparity(struct cyttsp4_core_data *cd,unsigned long disparity_val)
{
	u8 cmd_buf[CY_CMD_OP_SET_PARAM_CMD_SZ];
	u8 return_buf[CY_CMD_OP_SET_PARAM_RET_SZ];
	u8 scan_type;
...
static int cyttsp4_core_rt_resume(struct device *dev)
{
	...
	{
		tp_log_info("%s: enter,set holster_sensitivity when waking up,new status is %ld \n", __func__,glove_holster_status);
		msleep(60);
		rc = cyttsp4_set_disparity(cd,glove_holster_status);
		if (rc < 0) {
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	mutex_unlock(&cd->system_lock);
	glove_holster_status = disparity_val;
	cmd_buf[0] = CY_CMD_OP_SET_PARA;
	cmd_buf[1] = CY_OP_PARA_SCAN_TYPE;
	cmd_buf[2] = CY_OP_PARA_SCAN_TYPE_SZ;
	cmd_buf[3] = scan_type;
