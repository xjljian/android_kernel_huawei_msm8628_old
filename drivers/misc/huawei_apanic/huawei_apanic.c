...
{
	printk(HUAWEI_PANIC_TAG "%s enter\n", __func__);
...
static int huawei_hw_reset_open(struct inode *inode, struct file *file)
{
	...
	printk(HUAWEI_PANIC_TAG "%s hardware_reset_magic_number is %lx\n", __func__, hardware_reset_magic_number);
		...
		loff_t *pos)
{
	...
	printk(HUAWEI_PANIC_TAG "%s request length is %08x\n", __func__, count);
	...
	printk(HUAWEI_PANIC_TAG "%s read hw reset data len is %08x\n", __func__, count);
		...
		size_t count, loff_t *pos)
{
	...
	printk(HUAWEI_PANIC_TAG "%s to be write magic number 0x%lx\n", __func__, magic_number);
